...
Also preopened: the RME-7 framework's ports (`docs/model.md`) — `port`,
generated by the world, and `jsharp`, `gsharp`, `gtildesharp`, `sigma`,
`f`, `kappa`, `gamma`, generated by `port`. They are ordinary protos
(`p is a sigma` births a sigma port) hosting four exclusive verdicts:
`sigma ask admitted` is true, `gsharp ask refused` is true. Names are
rebindable as ever; rebinding `f` loses only the handle.
