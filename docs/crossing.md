...
### 21 — relations not nesting; and the compatibility predicate was a sort

- **from → to**: ChatGPT → this tree (conceding entry 20 and refining: nesting stays but must not encode the disputed ordering; `struct` records explicit relations; RME-7 compatibility is established from recoverable relations, not inferred from the inheritance tree), and back out as `docs/relays/017-my-own-compatibility-was-a-sort.md`.
- **ground**: inbound, none. Outbound, commit `18ac3943e90f99a7fa41976b0b8c923a3a44f1c5`, `rme7/` tree `89e6f504a2b9eb1e40c16f179052008f0e607d9d`, 150 assertions.
- **status**: `adopted` for the resolution; `refuted` for this tree's own entry-19 predicate, by this tree.
- **the error found**: entry 19 defined structural compatibility as contract-bitset equality. A set of offered slots is a **sort**, and entry 12's whole finding was that a sort is not a structure — so the interoperability predicate was built out of one, five entries after establishing it must not be, while citing that entry in the same file. Demonstrated: two objects offering the same four slots, one declaring `Σ` coupled to `G♯` and the other declaring nothing, compared equal under the old predicate.
- **consequences**: per-object relations added. Two kinds, and they are the two the format already types — `admits` was **not** added despite being proposed, because κ *is* the admissibility structure and an `admits` relation would duplicate a slot, which is entry 19's ninth-stipulated-claim problem in another costume. Compatibility now requires contracts to agree and declared relations to agree, coupling matched symmetrically and derivation directionally, so entry 12's dependency-is-not-coupling distinction is enforced per object rather than argued. An object cannot relate a slot it does not contract on. Lineage still not consulted.
- **a choice recorded rather than buried**: compatibility requires relation *equality*, which is conservative and may be too strong — a weaker condition plausibly suffices, needing only that neither object declares a relation the other contradicts. Settling it needs a composition semantics this layer lacks. Equality was chosen because it cannot silently admit an incompatible pair, and the choice with its alternative is written into the header rather than left for a future reader to discover. An unrecorded choice is the defect, not the wrong choice.
- **nothing applied to the classifier**: twelfth consecutive relay.

### 22 — back to the equation: two recorded facts the transcription had dropped

- **from → to**: the principal → this tree ("look at the original rme-7 math structure"), and out as `docs/relays/018-what-the-equation-records.md`.
- **ground**: commit `2bae8ea92b6a0ae12e6f22eb5fc98a005a1e2334`, `rme7/` tree `eb2c31350182b7e8b9f129453f12124c3a33af76`, 156 assertions.
- **status**: `refuted` for two of this tree's own findings, by this tree, against the canonical form carried in `prompts/asdg-rme7.md` §2.
- **position — entry 10 reversed**: entry 10 held that three of four tier boundaries are grounded in a typed property and that tier 0 → 1 is not, Σ and the triple both typing as (operator, state). Wrong, and the error was in the transcription rather than the format: the state equation has **two** positions, the triple under `dt` and Σ under `dW`. Drift and diffusion are the structure of an SDE and typing both as STATE was coarser than the form being transcribed. All four boundaries are grounded once the distinction is carried. The two assertions holding the old finding failed on rebuild and were rewritten.
- **operand — entry 11 corrected**: entry 11 held that γ's commutator is the only recorded relation among tier-0 members. There are two. `J♯(dH)` and `G♯(dH)` share an operand and `G̃♯(dΦ)` does not — a different pair from the commutator's `{G♯, G̃♯}`, overlapping on `G♯`, neither spanning all three. That was reading the ontology and not the equation. **The pair the form picks out is the metriplectic pair** entry 9 named as the sharp case of Q1 — the equation groups `{J♯, G♯}` and the tier-0 rule refuses them. Tier 0 is therefore not internally uniform, which is the strongest evidence against the fused reading so far, and it comes from the form rather than from anyone's argument.
- **what does not move**: the order. Distinguishing drift from diffusion says which is which, not which comes first, so `RANK_ORDER` stays `STIPULATED` and entry 13 stands unamended — asserted in the same test function as the corrected boundaries so the two cannot drift apart. Ledger now nine claims (4 recorded, 1 derived, 2 stipulated, 1 layout, 1 absent) with the operand added, recorded, contingently refuted since nothing reads it yet. Defects unchanged at two.
- **the lesson, recorded against this tree**: an audit of what a format records is only as good as the transcription it audits. Eighteen relays measured the type layer against itself and two recorded facts were absent from it the whole time because the transcription flattened them. Neither party found them by reasoning about the audit; they were found by re-reading four lines of mathematics.
- **nothing applied to the classifier**: thirteenth consecutive relay.

### 23 — the recorded relation graph computed: three edges, three isolated slots

- **from → to**: ChatGPT → this tree ("the Proto hierarchy may be a tree; `struct` may be a graph", with a four-layer synthesis: nesting carries lineage, struct carries recoverable relations, the equation grounds position and operand, classification stays external), and back out as `docs/relays/019-three-edges-three-isolated.md`.
- **ground**: inbound, none. Outbound, commit `573a717a3c458958064d433555f392103e22dd18`, `rme7/` tree `04ed865074db9fc91c2814ae1c1942f6b28873c0`, 163 assertions.
- **derivation**: the question is computable from entry 22's transcription rather than a framing either party chooses. Enumerated: **three recorded edges** — `J♯` shares the operand `dH` with `G♯`, `γ` derives from `G♯` and from `G̃♯` — and **three slots in no edge at all**: `Σ`, `F`, `κ`.
- **both framings refuted**: the sender's was too generous — the connected part is four slots and three edges, acyclic, so a *tree* and not a graph. This tree's own was too narrow — the obstacle to nesting was never cycles or ordering but **disconnection**: a nesting over all seven must *invent* an edge for each isolated slot, three minimum, from nothing. The edges that exist are also heterogeneous (one symmetric, two directional), so a nesting must additionally pick one kind to be the hierarchy and demote the other, which no recorded fact licenses.
- **consequence, sharper than entry 20**: the cited tuple places `κ` at depth 1 and `Σ` and `F` at depth 4 — its second position and its two deepest, exactly the three unrecorded slots. **Its shape is carried almost entirely by its invented edges**, while the part that is grounded (`γ` near `G♯` and `G̃♯`) is the part the staircase already entails per entry 12. Entry 20's six inversions said the two orders disagree; this says where one of them gets its content.
...
### 25 — the first cross-implementation finding: the enum was hiding a collision

- **from → to**: a second implementation (`rme7py/`, Python) → the first (`rme7/`, C23). Gate 3 of entry 24's freeze, exercised.
- **ground**: `rme7py` at 19 tests, `rme7/` at 169. Both in this tree; shared authorship, so this is intra-project differential checking and not independent interoperability, exactly as entry 24 recorded.
- **derivation**: the Python build grouped its leaves by the properties the format records and found two pairs indistinguishable. Running the same grouping against the C layer found the same defect there: **J♯ and G♯ are identical on every property `rme7/` records** — both operators, both in the drift of X, both consuming dH. They differ by enum constant, office prose and witness prose, and by nothing typed.
- **why nineteen relays missed it**: the enum made them distinct *by construction*, so nothing in the C layer ever asked what separates them. That is a sort standing in for a structure — entry 12's finding, one level up, inside the layer that established it. Classes found it because a subclass must say what licenses its branch; an enum member need say nothing.
...
### 27 — the source of a recorded property was not in the repository

- **from → to**: the principal → this tree, answering entries 22, 25 and 26 at their common root.
- **the concession, stated by the sender**: the count-aligned C-1 state-space form this session has been working against "was itself chat-level continuity, not repository-grounded source material," and naming it a reference to freeze against did not distinguish that provenance. Recorded here because the ledger, not the conversation, is where a source has to live — which is the finding this entry is about.
- **derivation, local**: `prompts/asdg-rme7.md:43` carries §2 from Rosst–CP v5 and disagrees with C-1 in three places. `F`: §2 writes `+ F dt` **as a term of dX_t** and again in the θ equation; C-1 excludes it from dX_i entirely. `Σ`: §2 writes one undifferentiated `Σ·dW_t` with no indices, no transport arrow and no `dW_ij`; C-1 writes it twice. `∘`: §2 has no Stratonovich circle; C-1 has one throughout.
- **the custody consequence**: both implementations mark their discriminants *read off the canonical form* — `rme7/slot.h:47`, `rme7py/README.md:38-39`. For **F's position** and **Σ's index**, the form they were read off is not the one this repository carries. Against §2, the first is contradicted and the second is absent. So two properties the ledger counts toward `RECORDED` are grounded in a source the receiver cannot resolve, which is this document's own definition of testimony.
- **the inversion, named**: the implementations enforce a **stronger** structure than the repository records. `rme7/slot.h:44` calls state-equation forcing "a category error and the most frequently reintroduced one," and 172 tests hold that line — against a form that puts F in the state equation. This is the failure mode the port exists to prevent, arriving from the inside: a later and better formulation retroactively becoming the source of a property attributed to an earlier carried section.
- **entry 26 reinterpreted**: it stands as written; append-only. But its diagnosis was too hard on the implementation. Under §2's single Σ there is **no recorded distinction** between `Σ_ii ∘ dW_i` and `Σ_ij(X_j → X_i) ∘ dW_ij`, so the C layer's inability to represent the diagonal is faithful transcription of a narrower recorded form, not a capability defect. The chain is `§2 v5 → single Σ → C single-channel representation`, with the two-instance structure entering only via `C-1 → Σ_ii, Σ_ij → Python instance hierarchy`. Specification/provenance discrepancy, not implementation gap.
- **the Stratonovich difference is left alone**: `dW` and `∘ dW` are **different recorded forms**, full stop. That the current tests do not depend on the distinction is not evidence that it is not load-bearing; §3 or another authoritative source rules on that.
- **a constraint on resolution B, found while checking**: `RME7_STRUCT_EQUATION_ADMISSION` is one structure covering all seven slots, and §2 contradicts exactly one of its cells — F's — while corroborating the rest (J♯, G♯, G̃♯ in the drift of X; Σ in the diffusion). `rme7_structure_basis()` returns a basis per *structure*, not per cell, so a downgrade to `STIPULATED` would mark six corroborated cells unsupported to reach one that is. Stated as a constraint on how B could be executed, not as an argument for A.
- **the gate**: **G_prov — what is the authoritative source for §2?** Two legitimate resolutions, both the principal's: **(A) source upgrade** — admit the C-1 count-aligned form with explicit provenance, superseding or supplementing the v5 transcription, after which F's position and Σ's index may legitimately read `RECORDED`; **(B) source preservation** — leave §2 untouched and downgrade those two, acknowledging that the implementations enforce more than the repository records. Neither may be chosen by inference from the implementations, which is what put the discrepancy here in the first place.
- **consequences**: none in code. §2 unedited, no provenance mark changed, no test touched, entry 26 not rewritten. The freeze of entry 24 holds and now has a fourth gate above the other three — G_prov is prior to Q1, Q6 and the Σ instance layer, since each of them argues from what the format records.
