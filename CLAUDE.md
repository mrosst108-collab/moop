...
The pipeline is lex (`src/lexer.{h,c}`) → parse (`src/parser.{h,c}`) → eval (`src/eval.{h,c}`), driven line-by-line by the REPL in `src/main.c`. One grammar rule: `statement := chain ["is" [chain]]`, `chain := term (op term)*`, left-associative — `a ask b ask c` is a pipeline. A trailing `is` opens an indented block (pythonic whitespace): one chain per line, blank line closes, the body is a sequence whose last value answers; indentation is handled by the reader, never the parser.
...
- `->` message passing: `x -> generate` births a proto from x (the built-in generative message), `x -> maybe` observes x's reversible body; other messages look up x's hosted table (delegating; the original receiver stays the one addressed).
...
- `is` asymmetric identity, two designator shapes: `name is chain` binds a name (right side evaluates now; names are the mutable user-layer handle, rebinding is fine); `receiver -> message is chain` teaches — the body is stored *unevaluated* as a chain and runs at each send, addressed to the receiver. Re-teaching replaces the chain; `generate`/`maybe`/`value` are innate and refuse redefinition.
