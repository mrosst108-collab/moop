...
- `src/` — interpreter sources. All `.c` files in `src/` are compiled and linked into the single `moop` binary (`src/moop.h` holds the version constant). `src/tapeloop.{h,c}` is the computational core; `src/gates.{h,c}` the reversible operators; `src/logic.{h,c}` the irreversible operators; `src/ram.{h,c}` user-facing memory; `src/actor.{h,c}` the actor runtime; `src/proto.{h,c}` the generative proto hierarchy; `src/rme7.{h,c}` the RME-7 slots on that ladder; `src/encode.{h,c}` the value-onto-tapes encoding; `src/lexer.{h,c}`, `src/parser.{h,c}`, `src/eval.{h,c}` the interpreter pipeline.
...
- `attribution/` — the state-versus-parameter attribution experiment against an executable baseline (target: cJSON's history, read-only and independent of this work). Its README is the frozen protocol: Q, rubric, baseline, hypotheses. Follow the order there; never alter Q after a baseline answer is seen.
- `reddit/` — a reddit in C23 shaped by RME-7, independent of the interpreter: a track struct (state `X`, generator `θ`) with the seven primitives as slots (function pointers with default occupants), `reddit_port` as the only two-track function, and tests that hold each slot's constraint and no-capture. Own Makefile: `cd reddit && make test`. Its `README.md` carries the mapping at role-plus-constraint level, every product prediction frozen before it was built (r/all, comments, users, delegation, rank-weighted ranking), and the three live-service freezes (identity bound at ingress, one serialized transcript order, wall time as recorded `tick` lines, downtime is not time) that `reddit serve` implements over a Unix socket. Persistence is the transcript; there is no other storage.
