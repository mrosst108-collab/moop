...
Current state: core, gates, logic, RAM, actors, protos, encoding, lexer, parser, and an evaluator with user-defined messages and gate-backed bijections are implemented and tested — moop is a working prototype-based OOP language (teaching, delegation, override, `self`, late binding) whose `<->` compiles to real gates. Interpreter bodies use 8/13-cell loops (values 0..255). Not implemented (and honestly erroring): number↔number bijections (definable invertible functions), running files, reflection (reading taught chains back as values). The REPL deliberately reports what it cannot do rather than pretending — keep that honesty: never stub behavior in a way that silently looks functional.
