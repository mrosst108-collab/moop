...
- `is` asymmetric identity, two designator shapes: `name is chain` binds a name (right side evaluates now; names are the mutable user-layer handle, rebinding is fine); `receiver -> message is chain` teaches — the body is stored *unevaluated* as a chain and runs at each send with `self` bound to the receiver. Re-teaching replaces the chain; `generate`/`maybe` are innate and refuse redefinition. The only keyword — matched exactly, `island` stays a word.
