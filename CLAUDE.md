...
Current state: core, gates, logic, RAM, actors, protos, lexer, parser, and an evaluator with user-defined messages are implemented and tested — moop is a working prototype-based OOP language (teaching, delegation, override, `self`, late binding). Not implemented (and honestly erroring): `<->`, running files, reflection (reading taught chains back as values), and the value encoding onto tape states. The REPL deliberately reports what it cannot do rather than pretending — keep that honesty: never stub behavior in a way that silently looks functional.
