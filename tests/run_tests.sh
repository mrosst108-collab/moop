...
# the RME-7 framework: port protos generated by the world at startup
check "port is generated by the world" "1" "$(printf 'port <- world\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "primitives are generated by port" "7" "$(printf 'jsharp <- port\ngsharp <- port\ngtildesharp <- port\nsigma <- port\nf <- port\nkappa <- port\ngamma <- port\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "sigma is the port: admitted" "1" "$(printf 'sigma ask admitted\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "a G-sharp port is capture: refused" "1" "$(printf 'gsharp ask refused\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "gamma is derived: absent" "1" "$(printf 'gamma ask absent\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "a G-tilde port is open" "1" "$(printf 'gtildesharp ask open\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "verdicts are exclusive" "1" "$(printf 'sigma ask refused\nquit\n' | "$BIN" 2>/dev/null | grep -c false)"
check "a port born from sigma is a sigma port" "1" "$(printf 'p is a sigma\np ask admitted\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "a port born from port alone is open" "1" "$(printf 'p is a port\np ask open\nquit\n' | "$BIN" 2>/dev/null | grep -c true)"
check "verdicts belong to ports, not the world" "error: nothing in the lineage hosts \"admitted\"" "$(printf 'world ask admitted\nquit\n' | "$BIN" 2>&1 >/dev/null)"
check "port bodies are bodies: mirrors works on them" "1" "$(printf 'box is a world\nsigma mirrors box\nquit\n' | "$BIN" 2>/dev/null | grep -c 'a proto')"
